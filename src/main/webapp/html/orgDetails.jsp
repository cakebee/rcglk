...
<body style="background-color: #E5E8E8;">
  <div class="container-fluid">
    ...
    <div class="row">
      ...
      <div class="formbox">
          <ul>
			<li>名称:{{org.orgName}}</li>
			<li>职责:{{org.orgDuty}}</li>
			<li>时段:{{org.orgBegin}} 至  {{org.orgEnd}}</li>
			<li>荣誉:{{org.orgHonor}}</li>
			<li>说明:{{org.orgIntro}}</li>
		</ul>
		</div>
      </div>
  </div>
...
app.controller('ViewCtrl', function($scope) {
	...
	console.log(url)
	var orgId = url.split('?')[1];
//	$scope.pri = [{"prizeName": "aaa","prizeLevel":"aa","prizeDate":"aa","status":"aa"}]
//	console.log($scope.pri)
		$.ajax({
	        url : "/rcglk/Studentsorg/"+orgId ,
	        type : "GET",
	        dataType:"json",
	        async : false,
	        //向后端发送的数据
	        success:function(data) {
	        	orgInfo=data.extend.studentsprize;
	        		var da = orgInfo.BeginDate;
	        		da = new Date(da);
	        	    var year = da.getFullYear();
	        	    var month = da.getMonth()+1;
	        	    var date = da.getDate();
	        	    orgInfo.BeginDate=([year,month,date].join('-'));
	        	    var da = orgInfo.EndDate;
	        		da = new Date(da);
	        	    var year = da.getFullYear();
	        	    var month = da.getMonth()+1;
	        	    var date = da.getDate();
	        	    orgInfo.EndDate=([year,month,date].join('-'));
	        	console.log(orgInfo)
	        },
	        error:function(data){
	        	console.log(data)
